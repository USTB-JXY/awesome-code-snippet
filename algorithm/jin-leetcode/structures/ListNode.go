...
func ListToIntArray(head *ListNode) []int {
	limit := 100
	times := 0
	res := []int{}
	for head != nil {
		times++
		if times > limit {
			msg := fmt.Sprintf("链条深度超过%d，可能出现环状链条。请检查错误，或者放宽limit 的限制。", limit)
			panic(msg)
		}
...
func IntArrayToList(nums []int) *ListNode {
	if len(nums) == 0 {
		return nil
	}
	l := &ListNode{}
