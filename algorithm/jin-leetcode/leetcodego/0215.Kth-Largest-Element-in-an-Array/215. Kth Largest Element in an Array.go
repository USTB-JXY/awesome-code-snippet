...
func myfindKthLargest1(nums []int, k int) int {
	sort.Ints(nums)
	return nums[len(nums)-k]
}
...
func myfindKthLargest(nums []int, k int) int {
	m := len(nums) - k + 1
	return myselectSmallest(nums, 0, len(nums)-1, m)
}

func myselectSmallest(nums []int, l, r, m int) int {
	if l >= r {
		return nums[l]
	}
	q := mypartition(nums, l, r)
	k := q - l + 1
	if k == m {
		return nums[q]
	}
	if m < k {
		return myselectSmallest(nums, l, q-1, m)
	} else {
		return myselectSmallest(nums, q+1, r, m-k)
	}
...
func mypartition(nums []int, l, r int) int {
	k := l + rand.Intn(r-l+1)
	nums[k], nums[r] = nums[r], nums[k]
	i := l - 1
	for j := l; j < r; j++ {
