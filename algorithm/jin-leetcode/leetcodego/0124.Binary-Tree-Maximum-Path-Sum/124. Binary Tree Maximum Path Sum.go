...
func mymaxPathSum(root *TreeNode) int {
	if root == nil {
		return 0
	}
	leftval, rightval := mymaxPathSum(root.Left), mymaxPathSum(root.Right)
	return max(root.Val+leftval+rightval, max(leftval, rightval))
}
