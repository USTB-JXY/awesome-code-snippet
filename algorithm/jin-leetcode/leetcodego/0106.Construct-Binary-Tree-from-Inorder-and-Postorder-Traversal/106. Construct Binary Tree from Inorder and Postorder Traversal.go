...
//	    /  \
//	   15   7
func mybuildTree2(inorder []int, postorder []int) *TreeNode {
	...
	for pos, val := range inorder {
		if val == root.Val {
			root.Left = mybuildTree2(inorder[:pos], postorder[:pos])
			root.Right = mybuildTree2(inorder[pos+1:], postorder[pos:len(postorder)-1])
			break
		}
	}
	return root
