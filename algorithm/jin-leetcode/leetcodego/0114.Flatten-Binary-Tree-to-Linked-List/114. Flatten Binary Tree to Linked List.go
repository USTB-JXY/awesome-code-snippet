...
func myflatten(root *TreeNode) {
	list := myprorder(root)
	for i := 0; i+1 < len(list); i++ {
		prev, next := list[i], list[i+1]
		prev.Left, prev.Right = nil, next
	}
	return
}
func myprorder(root *TreeNode) []*TreeNode {
	res := []*TreeNode{}
	if root != nil {
		res = append(res, root)
		res = append(res, myprorder(root.Left)...)
		res = append(res, myprorder(root.Right)...)
	}
	return res
}
